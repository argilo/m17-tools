...
int sock_start(std::string LocalAddress, unsigned short LocalPort)
{
    ...
    if (CUDPSocket::lookup(LocalAddress, LocalPort, m_sockaddr,
                           m_sockaddrLen) != 0) {
                  ...
                  int viterbi_cost)
{
    ...
    switch (frame.type) {
    case FrameType::LSF: {
        std::copy(frame.lsf.begin(), frame.lsf.begin() + 28,
                  M17packet.LICH.begin());
    ...
    case FrameType::STREAM: {
        ...
        if (viterbi_cost < 70 &&
            (fromchar(frame.stream[0], frame.stream[1]) & 0x8000)) {
        ...
        if (viterbi_cost < 80) {
            std::copy(frame.stream.begin() + 2, frame.stream.begin() + 18,
                      M17packet.Payload.begin());
            stream_crc.reset();
            for (size_t i = 0; i != sizeof(M17_IP); ++i) {
                stream_crc(((char *)&M17packet)[i]);
            }
            auto crc_bytes = stream_crc.get_bytes();
            std::copy((char *)&crc_bytes, (char *)&crc_bytes + 2,
                      (char *)&M17packet.CRC);
            if (!m_socket->write(
                    (uint8_t *)std::string((char *)&M17packet,
                                           (char *)&M17packet + sizeof(M17_IP))
                        .c_str(),
                    sizeof(M17_IP), sockaddr, sockaddrLen)) {
...
struct Config {
    ...
    static std::optional<Config> parse(int argc, char *argv[])
    {
        ...
        desc.add_options()("help,h", "Print this help message and exit.")(
            ...
            "addr,A",
            po::value<std::string>(&result.addr)->default_value("127.0.0.1"),
        ...
        if (vm.count("help")) {
            std::cout << "Read M17 baseband from STDIN and write M17 IP "
                         "packets to M17-Gateway\n"
                      << desc << std::endl;
