...
struct Config {
    ...
    static std::optional<Config> parse(int argc, char *argv[])
    {
        ...
        desc.add_options()("help,h", "Print this help message and exit.")(
            ...
            "Transmit/PTT activate action/command eg: toggle gpio on (default "
            "is no action).")(
            "transmit-off,O",
            ...
            po::value<std::string>(&result.event_device)
                ->default_value(
                    "/dev/input/by-id/"
                    "usb-C-Media_Electronics_Inc._USB_Audio_Device-event-if03"),
            "event device (default is C-Media Electronics Inc. USB Audio "
            "Device).")("encrypt,K", po::value<std::string>(&result.CKEY),
                        "hexadecimal string for AES 128, 192 or 256 Key "
                        "(default is no encryption).")(
            "key,k", po::value<uint16_t>(&result.key)->default_value(385),
            "Linux event code for PTT (default is RADIO).")(
            "bin,x", po::bool_switch(&result.bin),
            "output packed dibits (default is rrc).")(
            "rrc,r", po::bool_switch(&result.rrc),
            "output rrc filtered and scaled symbols (default).")(
            "sym,s", po::bool_switch(&result.sym),
            "output symbols (default is rrc).")(
            "bert,B", po::bool_switch(&result.bert),
            "output a bit error rate test stream (default is read audio from "
            "STDIN).")("invert,i", po::bool_switch(&result.invert),
                       "invert the output baseband (only for rrc)")(
        ...
        if (vm.count("help")) {
            std::cout
                << "Read audio from STDIN and write baseband M17 to STDOUT\n"
                << desc << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    if (!config->bert) {
        auto lsf =
            send_lsf(config->source_address, config->destination_address);
        ...
        while (running) {
            int16_t sample;
            if (!std::cin.read(reinterpret_cast<char *>(&sample), 2))
                break;
            if (!queue.put(sample, std::chrono::seconds(300)))
                break;
        }
...
int main(int argc, char *argv[])
{
    ...
    while (running) {
        if (!(bitstream_queue->get(bits, 1s))) {
            ...
            std::clog << "bitstream queue is closed; done transmitting."
                      << std::endl;
            running = false;
            break;
        }
        ...
        if (config->bitstream) {
            ...
            if (index == bitstream.size()) {
                auto baseband = M17Modulator::symbols_to_baseband(
                    M17Modulator::bytes_to_symbols(bitstream));
                for (auto b : baseband)
                    std::cout << uint8_t((b & 0xFF00) >> 8)
                              << uint8_t(b & 0xFF);
                std::cout.flush();
            }
        }
