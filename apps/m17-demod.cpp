...
void dump_type(uint16_t type)
{
    ...
    std::cerr << " CAN:" << std::dec << std::setw(2) << std::setfill('0')
              << int((type & 0x780) >> 7);
}

uint8_t last_FN[2];

template <typename T, size_t N> bool dump_lsf(std::array<T, N> const &lsf)
{
    ...
    if (display_lsf) {
        ...
        std::cerr << ", CRC: " << std::hex << std::setw(4) << std::setfill('0')
                  << crc;
...
bool demodulate_audio(mobilinkd::M17FrameDecoder::audio_buffer_t const &audio,
                      int viterbi_cost)
{
    bool result = true;
    ...
    if (noise_blanker && viterbi_cost > 80) {
        ...
        if (enc_key && frame_has_enc) { // if -K provided and ENC bits set
            ...
            std::cout.write((const char *)buf.data(), 320);
        } else if (frame_has_enc &&
                   !enc_key) { // if -K isn't provided but ENC bits set - MUTE
            buf.fill(0);
            std::cout.write((const char *)buf.data(), 320);
            std::cout.write((const char *)buf.data(), 320);
        } else {
...
bool decode_packet(
    mobilinkd::M17FrameDecoder::packet_buffer_t const &packet_segment)
{
    if (packet_segment[25] & 0x80) // last frame of packet.
    {
        ...
        std::cerr << "\nPacket checksum error: " << std::hex << checksum
                  << std::dec << std::endl;
    ...
    if (frame_number != packet_frame_counter) {
        std::cerr << "\nPacket frame sequence error. Got " << frame_number
                  << ", expected " << packet_frame_counter << "\n";
        return false;
    }
    ...
    mobilinkd::M17FrameDecoder::packet_buffer_t const &packet_segment)
{
    if (packet_segment[25] & 0x80) // last packet;
    {
        ...
        std::cout.write((const char *)&current_packet.front(),
                        current_packet.size());
...
bool handle_frame(mobilinkd::M17FrameDecoder::output_buffer_t const &frame,
                  int viterbi_cost)
{
...
void diagnostic_callback(bool dcd, FloatType evm, FloatType deviation,
                         FloatType offset, bool locked, FloatType clock,
                         int sample_index, int sync_index, int clock_index,
                         int viterbi_cost)
{
    if (debug) {
        std::cerr << "\rdcd: " << std::setw(1) << int(dcd)
                  << ", evm: " << std::setfill(' ') << std::setprecision(4)
                  << std::setw(8) << evm * 100 << "%"
                  << ", deviation: " << std::setprecision(4) << std::setw(8)
                  << deviation << ", freq offset: " << std::setprecision(4)
                  << std::setw(8) << offset << ", locked: " << std::boolalpha
                  << std::setw(6) << locked << std::dec
                  << ", clock: " << std::setprecision(7) << std::setw(8)
                  << clock << ", sample: " << std::setw(1) << sample_index
                  << ", " << sync_index << ", " << clock_index
                  << ", cost: " << viterbi_cost;
    }

    if (!dcd &&
        prbs.sync()) { // Seems like there should be a better way to do this.
        prbs.reset();
    }
    ...
    if (prbs.sync() && !quiet) {
        ...
        std::cerr << "BER: " << std::fixed << std::setprecision(6) << ber
                  << " (" << prbs.bits() << ")";
    }
    std::cerr << std::flush;
}
...
struct Config {
    ...
    static std::optional<Config> parse(int argc, char *argv[])
    {
        ...
        desc.add_options()("help,h", "Print this help message and exit.")(
            ...
            "noise-blanker,b", po::bool_switch(&result.noise_blanker),
            "noise blanker -- silence likely corrupt audio")(
            "lsf,l", po::bool_switch(&result.lsf),
            "display the decoded LSF")("bin,x", po::bool_switch(&result.bin),
                                       "input packed dibits (default is rrc).")(
            "rrc,r", po::bool_switch(&result.rrc),
            "input rrc filtered and scaled symbols (default).")(
            "sym,s", po::bool_switch(&result.sym),
            "input symbols (default is rrc).")(
            "encrypt,K", po::value<std::string>(&result.CKEY),
            "hexadecimal string for AES 128, 192 or 256 Key (default is no "
            "encryption).")("verbose,v", po::bool_switch(&result.verbose),
                            "verbose output")(
            "debug,d", po::bool_switch(&result.debug),
            "debug-level output")("quiet,q", po::bool_switch(&result.quiet),
        ...
        if (vm.count("help")) {
            std::cout
                << "Read M17 baseband from STDIN and write audio to STDOUT\n"
                << desc << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    while (std::cin) {
        ...
        case InputType::BIN: {
            char c;
            std::array<uint8_t, 1>
                packedSymbols;               // dibit packed byte to read in
