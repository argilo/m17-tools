...
struct Config {
    ...
    static std::optional<Config> parse(int argc, char* argv[])
    {
        ...
        desc.add_options()
            ...
            ("src,S",
    po::value<std::string>(&result.source_address)->required(), "transmitter
    identifier (your callsign).")
            ("dest,D", po::value<std::string>(&result.destination_address),
                "destination (default is broadcast).")
                        ("transmit-on,T",
    po::value<std::string>(&result.tx_on_cmd)->default_value(""), "Transmit/PTT
    activate action/command eg: toggle gpio on (default is no action).")
                        ("transmit-off,O",
    po::value<std::string>(&result.tx_off_cmd)->default_value(""), "Transmit/PTT
    deactivate action/command eg: toggle gpio off (default is no action).")
            ...
            ("event,e",
    po::value<std::string>(&result.event_device)->default_value("/dev/input/by-id/usb-C-Media_Electronics_Inc._USB_Audio_Device-event-if03"),
                "event device (default is C-Media Electronics Inc. USB Audio
    Device).")
                        ("encrypt,K",po::value<std::string>(&result.CKEY),
    "hexadecimal string for AES 128, 192 or 256 Key (default is no
    encryption).")
            ("key,k", po::value<uint16_t>(&result.key)->default_value(385),
                "Linux event code for PTT (default is RADIO).")
            ("bin,x", po::bool_switch(&result.bin), "output packed dibits
    (default is rrc).")
            ("rrc,r", po::bool_switch(&result.rrc), "output rrc filtered and
    scaled symbols (default).")
            ("sym,s", po::bool_switch(&result.sym), "output symbols (default is
    rrc).")
            ("bert,B", po::bool_switch(&result.bert),
                "output a bit error rate test stream (default is read audio from
    STDIN).")
            ("invert,i", po::bool_switch(&result.invert), "invert the output
    baseband (only for rrc)")
        ...
        if (vm.count("help"))
        {
            std::cout << "Read audio from STDIN and write baseband M17 to
    STDOUT\n"
                << desc << std::endl;
    ...
    ". Must be 0-15." << std::endl; return std::nullopt;
        }

        if (result.sym + result.bin + result.rrc > 1)
        {
            std::cerr << "Only one of sym, bin or rrc may be chosen." <<
    std::endl; return std::nullopt;
        }
...
void transmit(std::shared_ptr<queue_t> &queue, const lsf_t &lsf)
{
    ...
    while (!queue->is_closed()) {
        int16_t sample;
        if (!queue->get(sample, std::chrono::milliseconds(3000)))
            break;
        audio[index++] = sample;
        if (index == audio.size()) {
            index = 0;
...
int main(int argc, char *argv[])
{
    ...
    while (!glfwWindowShouldClose(window)) {
...

        {
            ...
            ImGui::InputTextWithHint("SRC", "SRC CALLSIGN", str1, 7,
                                     ImGuiInputTextFlags_CharsUppercase);
            ImGui::InputTextWithHint("DST", "DST CALLSIGN", str2, 7,
                                     ImGuiInputTextFlags_CharsUppercase);
            ImGui::InputInt("CAN", &config->can);
            ImGui::Checkbox("Invert", &config->invert);
            ImGui::Checkbox("Encrypt", &config->encrypt);
            ImGui::InputText("AES Key", buf, 128,
            ...
            if (tx) {
                ...
                ImGui::PushStyleColor(
                    ...
                    (ImVec4)ImColor::HSV(0 / 7.0f, 0.4f, 0.6f));
            } else {
                ImGui::PushStyleColor(
                    ImGuiCol_Button,
                ...
                ImGui::PushStyleColor(
                    ImGuiCol_ButtonHovered,
                    (ImVec4)ImColor::HSV(2 / 7.0f, 0.4f, 0.6f));
            }
            ...
            if (btn) {
                ...
                if (config->source_address.length() > 4) {
                    if (!running) {
                        ...
                        std::string hash =
                            boost::algorithm::unhex(config->CKEY);
                        std::copy(hash.begin(), hash.end(), Key);
                        ...
                        auto lsf = send_lsf(config->source_address,
                        ...
                        thd = std::thread(transmit, std::ref(squeue),
                                          std::ref(lsf));

                        adc.startStream();

                        std::cerr << "m17-mod running. ctrl-D to break."
                                  << std::endl;
        ...
        }
        {
            ...
            if (ImGui::BeginCombo("input_audio",
                                  in_device_names[in_dev_id].c_str())) {
                for (int i = 0; i < in_device_names.size(); ++i) {
                    const bool isSelected = (in_dev_id == i);
                    if (ImGui::Selectable(in_device_names[i].c_str(),
                                          isSelected)) {
                        in_dev_id = i;
                        if (running) {
                            tx = false;
            ...
            ImGui::SetNextItemWidth(menuWidth);
            if (ImGui::BeginCombo("output_audio",
                                  out_device_names[out_dev_id].c_str())) {
                for (int i = 0; i < out_device_names.size(); ++i) {
                    const bool isSelected = (out_dev_id == i);
                    if (ImGui::Selectable(out_device_names[i].c_str(),
                                          isSelected)) {
                        out_dev_id = i;
                    }
