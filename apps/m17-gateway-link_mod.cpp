    0.002365929161655135,   -0.0006430502751187517, -0.003403189203405097,
    -0.005349511529163396,  -0.006126925416243605,  -0.005648131453822014,
    -0.004092346891623224,  -0.0018493784971116507, 0.0005756567993179152,
    0.002674727068399207,   0.00404219185231544,    0.004451886520053017,
    0.0038920145144327816,  0.002553421969211845,   0.0007766690889758685,
    -0.0010285696910973807, -0.0024733964729590865, -0.0032697038088887226,
    -0.0032843366522956313, -0.0025577136087664687, -0.0012851320781224025,
    0.00023319405581230247, 0.001661182944400927,   0.002699564567597445,
    0.0031468394550958484,  0.0029364388513841593,  0.0};
...
struct Config {
    ...
    static std::optional<Config> parse(int argc, char *argv[])
    {
        ...
        desc.add_options()("help,h", "Print this help message and exit.")(
            ...
            ("addr,A",
             po::value<std::string>(&result.audio_device)
                 ->default_value("127.0.0.1"),
             "Gateway repeater address (default is 127.0.0.1).")(
                "port,P",
                po::value<unsigned short>(&result.port)->default_value(17011),
                "Gateway repeater port (default is 17011).")(
                "transmit-on,T",
                po::value<std::string>(&result.tx_on_cmd)->default_value(""),
                "Transmit/PTT activate action/command eg: toggle gpio on "
                "(default is no action).")(
                "transmit-off,O",
                po::value<std::string>(&result.tx_off_cmd)->default_value(""),
                "Transmit/PTT deactivate action/command eg: toggle gpio off "
                "(default is no action).")(
                ...
                "event,e",
                po::value<std::string>(&result.event_device)
                    ->default_value("/dev/input/by-id/"
                                    "usb-C-Media_Electronics_Inc._USB_Audio_"
                                    "Device-event-if03"),
                "event device (default is C-Media Electronics Inc. USB Audio "
                "Device).")(
                ...
                "output a bit error rate test stream (default is read audio "
                "from STDIN).")(
                "invert,i", po::bool_switch(&result.invert),
                "invert the output baseband (ignored for bitstream)")(
                "verbose,v", po::bool_switch(&result.verbose),
                "verbose output")("debug,d", po::bool_switch(&result.debug),
                                  "debug-level output")(
                "quiet,q", po::bool_switch(&result.quiet),
                "silence all output");
        ...
        if (vm.count("help")) {
            std::cout << "Read IP packets from M17-Gateway and write M17 "
                         "baseband to STDOUT\n"
                      << desc << std::endl;
        ...
        if (result.debug + result.verbose + result.quiet > 1) {
            std::cerr << "Only one of quiet, verbose or debug may be chosen."
                      << std::endl;
            return std::nullopt;
        }
        ...
        if (result.can < 0 || result.can > 15) {
            std::cerr << "invalid channel access number (CAN) " << result.can
                      << ". Must be 0-15." << std::endl;
            return std::nullopt;
        }
...
void output_bitstream(std::array<uint8_t, 2> sync_word,
                      const bitstream_t &frame)
{
...
void output_eot()
{
    if (bitstream) {
        ...
        for (size_t i = 0; i != 10; ++i)
            std::cout << '\0'; // Flush RRC FIR Filter.
    } else {
...
lsf_t send_lsf(const std::string &src, const std::string &dest,
               const FrameType type = FrameType::AUDIO)
{
    using namespace mobilinkd;
    ...
    mobilinkd::LinkSetupFrame::encoded_call_t encoded_dest = {0xff, 0xff, 0xff,
                                                              0xff, 0xff, 0xff};
    if (!dest.empty()) {
        callsign.fill(0);
    ...
    auto rit =
        std::copy(encoded_dest.begin(), encoded_dest.end(), result.begin());
...
template <typename PRBS> bitstream_t make_bert_frame(PRBS &prbs)
{
...
void M17RTXLink::run()
{
    ...
    while (1) {
        ...
        if (ret > 0) {
            ...
            // Get MAGIC
            std::copy(command.begin(), command.begin() + 4,
                      packet.MAGIC.begin());
            int magic = (uint32_t)packet.MAGIC[0] << 24 |
                        (uint32_t)packet.MAGIC[1] << 16 |
                        (uint32_t)packet.MAGIC[2] << 8 |
                        (uint32_t)packet.MAGIC[3];

            if (mdebug) {
                std::cerr << "[DEBUG] MAGIC: "
                          << std::string(packet.MAGIC.begin(),
                                         packet.MAGIC.end())
                          << "\n";
            }
            ...
            if (magic == 0x4d313720) {
                ...
                std::copy(command.begin() + 36, command.begin() + 36 + 16,
                          packet.Payload.begin()); // CODEC2 PAYLOAD
                ...
                if (packet.FN == 0x00) {
                    ...
                    packet.SID =
                        fromchar(command[4], command[5]); // SID (not used...)

                    // Make LSF & Swap DST->SRC
                    std::copy(command.begin() + 6, command.begin() + 6 + 28,
                              packet.LICH.begin());
                    ...
                    // Decode SRC Callsign
                    mobilinkd::LinkSetupFrame::encoded_call_t encoded_src = {
                        0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
                    std::copy(packet.LICH.begin(), packet.LICH.begin() + 6,
                              encoded_src.begin());
                    auto src =
                        mobilinkd::LinkSetupFrame::decode_callsign(encoded_src);

                    // Encode SRC Callsign
                    mobilinkd::LinkSetupFrame::encoded_call_t encoded_dst{
                        0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
                    std::copy(packet.LICH.begin() + 6, packet.LICH.begin() + 12,
                              encoded_dst.begin());
                    auto dst =
                        mobilinkd::LinkSetupFrame::decode_callsign(encoded_dst);

                    if (mdebug) {
                        std::cerr
                            << "[DEBUG] FN: " << packet.FN
                            << " SRC: " << std::string(src.begin(), src.end())
                            << " DST: " << std::string(dst.begin(), dst.end())
                            << "\n";
                    }
                    ...
                    send_preamble();
                    lsf = send_lsf(std::string(src.begin(), src.end()),
                                   std::string(dst.begin(), dst.end()));

                    lich_segment = 0;
                    for (size_t i = 0; i != lich.size(); ++i) {
                        std::array<uint8_t, 5> segment;
                        std::copy(lsf.begin() + i * 5,
                                  lsf.begin() + (i + 1) * 5, segment.begin());
                        auto lich_segment = make_lich_segment(segment, i);
                ...
                std::copy(packet.Payload.begin(), packet.Payload.end(),
                          payload.begin());
                if (!qPayload.put(payload, std::chrono::seconds(300))) {
                    std::cerr << "QUEUE FULL BREAK\n";
                    break;
                }
            }
        } else {
...
void M17RTXLink::transmit()
{
    ...
    while (!qPayload.is_closed() && qPayload.empty())
