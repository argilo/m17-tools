...
struct ax25_frame {
    ...
    using pid_type = std::optional<uint8_t>;
...
private:
    ...
    static std::string parse_info(const std::string &frame, size_t pos)
    {
        ...
        for (int i = pos; i != frame.size() - 2; i++) {
            ...
            if (std::isprint(c)) {
                ...
                output << "0x" << std::setw(2) << std::setbase(16)
                       << int(uint8_t(c)) << ' ';
            }
        }
        return output.str();
    ...
    static uint16_t parse_fcs(const std::string &frame)
    {
        ...
        uint16_t tmp = ((uint8_t(frame[checksum_pos + 1]) << 8) |
    ...
    static repeaters_type parse_repeaters(const std::string &frame)
    {
        ...
        while (more) {
            ...
            more = fixup_address(repeater) and
                   (index + ADDRESS_LENGTH) < frame.length();
...
public:
    ax25_frame(const std::string &frame)
        : destination_(), source_(), repeaters_(), type_(UNDEFINED), info_(),
          fcs_(-1), crc_(0), pid_()
    {
        parse(frame);
    }
...
void write(std::ostream &os, const ax25_frame &frame)
{
    ...
    if (!repeaters.empty()) {
        os << "Via: ";
        std::copy(repeaters.begin(), repeaters.end(),
