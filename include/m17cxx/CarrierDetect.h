...
template <typename FloatType> struct CarrierDetect {
    ...
    CarrierDetect(std::array<FloatType, 3> const &b,
                  std::array<FloatType, 3> const &a, FloatType lock_level,
                  FloatType unlock_level)
    ...
    result_t operator()(FloatType value)
    {
        ...
        if (locked_ && (filtered > unlock_))
            locked_ = false;
        else if (!locked_ && (filtered < lock_))
            locked_ = true;
