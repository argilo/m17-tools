...
constexpr std::bitset<sizeof...(Is)> make_bitset(std::index_sequence<Is...>,
                                                 Tuple &&tuple)
{
 ...
 */
template <size_t N> constexpr size_t llr_limit() { return (1 << (N - 1)) - 1; }
 ...
 */
template <size_t N> constexpr size_t llr_size()
{
    return llr_limit<N>() * 6 + 1;
}

template <typename FloatType, size_t LLR>
constexpr std::array<std::tuple<FloatType, std::tuple<int8_t, int8_t>>,
                     llr_size<LLR>()>
make_llr_map()
{
...
template <class... Bools> constexpr auto make_bitset(Bools &&...bools)
{
...
template <typename FloatType, size_t LLR> auto llr(FloatType sample)
{
    ...
    auto it = std::lower_bound(
        symbol_map.begin(), symbol_map.end(), s,
        [](std::tuple<FloatType, std::tuple<int8_t, int8_t>> const &e,
           FloatType s) { return std::get<0>(e) < s; });

    if (it == symbol_map.end())
        return std::get<1>(*symbol_map.rbegin());
...
size_t puncture(const std::array<T, IN> &in, std::array<U, OUT> &out,
...
{
    if (value)
        set_bit_index(input, index);
    else
        reset_bit_index(input, index);
}
 ...
 */
template <typename T, size_t n> constexpr T to_int(uint8_t v)
{
    constexpr auto MAX_LOCAL_INPUT = (1 << (n - 1));
    constexpr auto NEGATIVE_OFFSET =
...
template <typename T, size_t N>
constexpr void to_byte_array(std::array<T, N> in,
                             std::array<uint8_t, (N + 7) / 8> &out)
{
    size_t i = 0;
    size_t b = 0;
...
struct PRBS9 {
    ...
    static constexpr uint8_t UNLOCK_COUNT =
        25; // bad bits in history required to unlock.
