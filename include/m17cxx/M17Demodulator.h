...
template <typename FloatType> struct Taps {
};

template <> struct Taps<double> {
...
template <typename FloatType> struct M17Demodulator {
    ...
    using diagnostic_callback_t =
        std::function<void(bool, FloatType, FloatType, FloatType, bool,
                           FloatType, int, int, int, int)>;
    ...
    BaseFirFilter<FloatType, detail::Taps<FloatType>::rrc_taps.size()>
        demod_filter{detail::Taps<FloatType>::rrc_taps};
    ...
    sync_word_t lsf_sync{{+3, +3, +3, +3, -3, -3, +3, -3},
                         32.f,
                         -31.f}; // LSF or STREAM (inverted)
    sync_word_t packet_sync{{3, -3, 3, 3, -3, -3, -3, -3},
                            31.f,
                            -31.f}; // PACKET or BERT (inverted)
    ...
    M17FrameDecoder::SyncWordType sync_word_type =
        M17FrameDecoder::SyncWordType::LSF;
    ...
    M17Demodulator(callback_t callback) : decoder(callback) {}
...
template <typename FloatType> void M17Demodulator<FloatType>::dcd_on()
{
...
template <typename FloatType> void M17Demodulator<FloatType>::dcd_off()
{
...
template <typename FloatType> void M17Demodulator<FloatType>::update_dcd()
{
    if (!dcd_ && dcd.dcd()) {
        // fputs("\nAOS\n", stderr);
        dcd_on();
...
template <typename FloatType> void M17Demodulator<FloatType>::do_unlocked()
{
 ...
 */
template <typename FloatType> void M17Demodulator<FloatType>::do_lsf_sync()
{
 ...
 *
 */
template <typename FloatType> void M17Demodulator<FloatType>::do_stream_sync()
{
 ...
 */
template <typename FloatType> void M17Demodulator<FloatType>::do_packet_sync()
{
 ...
 */
template <typename FloatType> void M17Demodulator<FloatType>::do_bert_sync()
{
    auto sync_index = packet_sync(correlator);
    auto sync_updated = packet_sync.updated();
    sync_count += 1;
...
void M17Demodulator<FloatType>::operator()(const FloatType input)
{
    ...
    if (!dcd_) {
        if (count_ % (BLOCK_SIZE * 2) == 0) {
            ...
            if (diagnostic_callback) {
                diagnostic_callback(
                    int(dcd_), dev.error(), dev.deviation(), dev.offset(),
                    (demodState != DemodState::UNLOCKED),
                    clock_recovery.clock_estimate(), sample_index,
                    sync_sample_index, clock_recovery.sample_index(),
                    viterbi_cost);
            }
            count_ = 0;
        }
    ...
    if (correlator.index() == 0) {
        ...
        } else if (need_clock_update_) // must avoid update immediately after
                                       // reset.
        {
    ...
    if (count_ % (BLOCK_SIZE * 5) == 0) {
        ...
        if (diagnostic_callback) {
            diagnostic_callback(
                ...
                (demodState != DemodState::UNLOCKED),
                clock_recovery.clock_estimate(), sample_index,
                sync_sample_index, clock_recovery.sample_index(), viterbi_cost);
        }
        dcd.update();
    }
}
