...
template <typename FloatType> struct Correlator {
    ...
    static constexpr std::array<FloatType, 3> b = {
        4.24433681e-05, 8.48867363e-05, 4.24433681e-05};
    static constexpr std::array<FloatType, 3> a = {1.0, -1.98148851,
                                                   0.98165828};
    ...
    std::tuple<FloatType, FloatType> outer_symbol_levels(size_t sample_index)
    {
        ...
        size_t max_count = 0;
        size_t index = 0;
        for (size_t i = sample_index; i < buffer_.size();
    ...
    template <typename F> void apply(F func, uint8_t index)
    {
...
template <typename Correlator> struct SyncWord {
    ...
    SyncWord(buffer_t &&sync_word, value_type magnitude_1,
             value_type magnitude_2 = std::numeric_limits<value_type>::lowest())
        : sync_word_(std::move(sync_word)), magnitude_1_(magnitude_1),
