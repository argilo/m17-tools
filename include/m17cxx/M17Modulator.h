...
/**
 * Asynchronous M17 modulator.  This modulator is initialized with the source
 * and destination callsigns.  It is then run by attaching an input queue and an
 * output queue.  The modulator reads 16-bit, 8ksps, 1-channel audio samples
 * from the input queue and an M17 bitstream (in 8-bit bytes, 4 symbols per
 * byte) to the output queue.
 *
 * The call to run(), which is used to attach the queues, returns immediately,
 * starting a new thread in a detached state.  run() returns a future, which may
 * contain error information if an exception is thrown.
 *
 * The modulator stops when the input queue is closed.
 *
 * The modulator starts in a paused state, discarding all input.
 *
 * The modulator is started by calling ptt_on().  This causes the preamble and
 * link setup frame to be sent.  The modulator then starts reading from the
 * input queue and writing the data stream to the output queue.
 *
 ...
 * signalling.
 *
 * Output will be bursty -- their is no throttling of the symbol stream.  As
 * soon as enough input samples are received to fill the M17 payload field, the
 * frame will be constructed and the symbol stream output on the queue.
 *
 ...
 *
 * The modulator will transition from PREAMBLE to LINK_SETUP to ACTIVE
 * automatically.
 *
 * The modulator transitions from ACTIVE to END_OF_STREAM when ptt_off() is
 * called.
 *
 ...
 * queue is closed.
 *
 * The modulator will emit at least 3 frames when ptt_on() is called: the
 * preamble, the link setup frame, and one audio frame with the EOS flag set.
 *
 * It is an error to close the input or output stream when the modulator is not
 * IDLE.
 *
 * @section Thread Safety
 *
 * Internally, the modulator is thread-safe.  It is running with a background
 * thread reading from and writing to thread-safe queues.  Externally, the
 * modulator expects that all API calls made synchronously as if from a single
 * thread of control.
 *
 * @section Convertion Functions
 *
 * There are two public static conversion functions provided to support
 * conversion of the output bitstream into either a symbol stream or into a
 * 48ksps baseband stream.
 */
struct M17Modulator {
public:
    using bitstream_queue_t =
        queue<uint8_t,
              96>; // 1 frame's worth of data, 48 bytes, 192 symbols, 384 bits.
    using audio_queue_t = queue<int16_t, 320>; // 1 frame's worth of data.
    using symbols_t = std::array<int8_t, 192>; // One frame of symbols.
    using baseband_t =
        std::array<int16_t, 1920>; // One frame of baseband data @ 48ksps
    using bitstream_t =
        std::array<uint8_t, 48>; // M17 frame of bits (in bytes).
...
private:
    ...
    using payload_t =
        std::array<uint8_t, 34>; // Bytes in the payload of a data frame.
    ...
    {
        LinkSetupFrame::encoded_call_t encoded_call = {0xff, 0xff, 0xff,
                                                       0xff, 0xff, 0xff};
    ...
    static std::array<int8_t, N / 2>
    bits_to_symbols(const std::array<T, N> &bits)
    {
    ...
    void send_link_setup(lich_t &lich)
    {
        ...
        for (size_t i = 0; i != lich.size(); ++i) {
            ...
            std::copy(lsf.begin() + i * 5, lsf.begin() + (i + 1) * 5,
                      segment.begin());
            auto lich_segment = make_lich_segment(segment, i);
            std::copy(lich_segment.begin(), lich_segment.end(),
                      lich[i].begin());
        }
    ...
    /**
     * Append the LICH and Convolutionally encoded payload, interleave and
     * randomize the frame bits, and output the frame.
     */
    ...
    /**
     * Assemble the audio frame payload by appending the frame number, encoded
     * audio, and CRC, then convolutionally coding and puncturing the data.
     */
     ...
     */
    void send_audio(const lich_segment_t &lich, uint16_t frame_number,
    ...
    void modulate()
    {
        ...
        while (audio_queue_->is_open() && bitstream_queue_->is_open()) {
            int16_t sample;
            if (!(audio_queue_->get(sample, 5s)))
                sample = 0; // May be closed.
            ...
            case State::ACTIVE:
                ...
                if (index == audio.size()) {
                    auto now = clock::now();
                    if (now - current > 40ms) {
                        std::clog << "WARNING: packet time exceeded"
                                  << std::endl;
                    }
                    current = now;
                    index = 0;
        ...
        if (state_ != State::IDLE)
            throw std::logic_error("queue closed when not IDLE");
...
public:
    M17Modulator(const std::string &source, const std::string &dest = "")
        : source_(encode_callsign(source)), dest_(encode_callsign(dest))
    {
    }
    ...
    /**
     * Start the modulator.  This starts a background thread and returns once
     * the thread has started and changed the state to IDLE.
     *
     ...
     */
    std::future<void> run(const std::shared_ptr<audio_queue_t> &input,
                          const std::shared_ptr<bitstream_queue_t> &output)
    {
        using namespace std::chrono_literals;
        ...
        auto result =
            std::async(std::launch::async, [this]() { this->modulate(); });
    ...
    /**
     * Activate the modulator.  This causes the modulator to transition from
     * IDLE to ACTIVE.  If the modulator is already ACTIVE, no action is taken.
     * If the modulator is not IDLE, return is delayed until the modulator
     * becomes IDLE (which may take up to 120ms), at which time the modulator is
     * returned to the ACTIVE state. Otherwise the modulator immediately
     * transistions from IDLE to ACTIVE. This will cause the preamble and link
     * setup frames to be emitted.
     *
    ...
    void ptt_on()
    {
        ...
        if (state_ == State::ACTIVE)
            return;
        while (state_ != State::IDLE && state_ != State::INACTIVE)
            std::this_thread::sleep_for(1ms);
        assert(state_ == State::IDLE); // Precondition violated -- one of the
                                       // queues was closed.
     ...
     * Stop the modulator.
     *
     * @pre ptt_on() was called and the modulator is in PREAMBLE, LINK_SETUP, or
     * ACTIVE state.
     */
    void ptt_off()
    {
        ...
        assert(state_ == State::PREAMBLE | state_ == State::LINK_SETUP |
               state_ == State::ACTIVE);

        // State must become active before we release PTT to ensure preamble and
        // LSF are sent.
        while (state_ != State::ACTIVE && state_ != State::INACTIVE)
            std::this_thread::sleep_for(1ms);
        assert(state_ == State::ACTIVE); // Precondition violated -- one of the
                                         // queues was closed.
    ...
    template <typename T, size_t N>
    static std::array<int8_t, N * 4>
    bytes_to_symbols(const std::array<T, N> &bytes)
    {
    ...
    static baseband_t symbols_to_baseband(const symbols_t &symbols)
    {
        ...
        static const auto rrc_taps = std::array<double, 79>{
            -0.009265784007800534,  -0.006136551625729697,
            -0.001125978562075172,  0.004891777252042491,
            0.01071805138282269,    0.01505751553351295,
            0.01679337935001369,    0.015256245142156299,
            0.01042830577908502,    0.003031522725559901,
            -0.0055333532968188165, -0.013403099825723372,
            -0.018598682349642525,  -0.01944761739590459,
            -0.015005271935951746,  -0.0053887880354343935,
            0.008056525910253532,   0.022816244158307273,
            0.035513467692208076,   0.04244131815783876,
            0.04025481153629372,    0.02671818654865632,
            0.0013810216516704976,  -0.03394615682795165,
            -0.07502635967975885,   -0.11540977897637611,
            -0.14703962203941534,   -0.16119995609538576,
            -0.14969512896336504,   -0.10610329539459686,
            -0.026921412469634916,  0.08757875030779196,
            0.23293327870303457,    0.4006012210123992,
            0.5786324696325503,     0.7528286479934068,
            0.908262741447522,      1.0309661131633199,
            1.1095611856548013,     1.1366197723675815,
            1.1095611856548013,     1.0309661131633199,
            0.908262741447522,      0.7528286479934068,
            0.5786324696325503,     0.4006012210123992,
            0.23293327870303457,    0.08757875030779196,
            -0.026921412469634916,  -0.10610329539459686,
            -0.14969512896336504,   -0.16119995609538576,
            -0.14703962203941534,   -0.11540977897637611,
            -0.07502635967975885,   -0.03394615682795165,
            0.0013810216516704976,  0.02671818654865632,
            0.04025481153629372,    0.04244131815783876,
            0.035513467692208076,   0.022816244158307273,
            0.008056525910253532,   -0.0053887880354343935,
            -0.015005271935951746,  -0.01944761739590459,
            -0.018598682349642525,  -0.013403099825723372,
            -0.0055333532968188165, 0.003031522725559901,
            0.01042830577908502,    0.015256245142156299,
            0.01679337935001369,    0.01505751553351295,
            0.01071805138282269,    0.004891777252042491,
            -0.001125978562075172,  -0.006136551625729697,
            -0.009265784007800534};
        static BaseFirFilter<double, std::tuple_size<decltype(rrc_taps)>::value>
