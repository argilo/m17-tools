...
template <uint16_t Poly = 0x5935, uint16_t Init = 0xFFFF> struct CRC16 {
    ...
    void operator()(uint8_t byte) { reg_ = crc(byte, reg_); }
    ...
    {
        auto crc = get();
        std::array<uint8_t, 2> result{uint8_t((crc >> 8) & 0xFF),
                                      uint8_t(crc & 0xFF)};
        return result;
    }
};
