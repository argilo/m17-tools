...
public:
    ...
    bool get(reference val, std::chrono::duration<Rep, Period> timeout =
    ...
    bool put(U &&val, std::chrono::duration<Rep, Period> timeout =
                          std::chrono::duration<Rep, Period>::max())
    {
        // Get the queue mutex.
        lock_type lock(mutex_);
        ...
        if (SIZE == size_) {
            ...
            while (SIZE == size_) {
                ...
                if (full_.wait_until(lock, expiration) ==
    ...
    bool is_open() const { return State::OPEN == state_; }

    bool is_closed() const { return State::CLOSED == state_; }
