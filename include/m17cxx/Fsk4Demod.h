...
inline const auto evm_b =
    std::array<double, 3>(0.02008337, 0.04016673, 0.02008337);
...
struct Fsk4Demod {
    ...
    using result_t = std::optional<
        std::tuple<double, double, int, double, double, double, double>>;

    BaseFirFilter<double, std::tuple_size<decltype(detail::rrc_taps)>::value>
        rrc = makeFirFilter(detail::rrc_taps);
    ...
    SymbolEvm<double, std::tuple_size<decltype(detail::evm_b)>::value>
        symbol_evm = makeSymbolEvm(makeIirFilter(detail::evm_b, detail::evm_a));
    ...
    Fsk4Demod(double sample_rate, double symbol_rate, double unlock_gain = 0.02,
              double lock_gain = 0.001)
        : sample_rate(sample_rate), symbol_rate(symbol_rate),
          ...
          dt(symbol_rate / sample_rate), ideal_dt(dt)
    {
        samples.fill(0.0);
    }
    ...
    result_t operator()(double sample, bool lock)
    {
        ...
        if (sample_now) {
            ...
            return std::make_tuple(prev_sample, phase_estimate, symbol, evm,
                                   estimated_deviation,
                                   estimated_frequency_offset, evm_average);
        }
