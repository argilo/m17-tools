 ...
 */
template <typename FloatType> class FreqDevEstimator
{
    using sample_filter_t = BaseIirFilter<FloatType, 3>;

    // IIR with Nyquist of 1/4.
    static constexpr std::array<FloatType, 3> dc_b = {0.09763107, 0.19526215,
                                                      0.09763107};
    static constexpr std::array<FloatType, 3> dc_a = {1., -0.94280904,
                                                      0.33333333};
...
public:
    ...
    void update()
    {
        ...
        offset_ = dc_filter_(
            std::max(std::min(max_ + min_, deviation_ * MAX_DC_ERROR),
                     deviation_ * -MAX_DC_ERROR));
        error_ = (std::sqrt(max_var_ / (max_count_ - 1)) +
                  std::sqrt(min_var_ / (min_count_ - 1))) *
                 0.5;
        if (deviation_ > 0)
