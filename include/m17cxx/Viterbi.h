...
constexpr std::array<std::array<uint8_t, (1 << Trellis_::k)>,
                     (1 << Trellis_::K)>
makeNextState(Trellis_)
{
    std::array<std::array<uint8_t, (1 << Trellis_::k)>, (1 << Trellis_::K)>
        result{};
    for (size_t i = 0; i != (1 << Trellis_::K); ++i) {
        for (size_t j = 0; j != (1 << Trellis_::k); ++j) {
            result[i][j] = static_cast<uint8_t>(
                update_memory<Trellis_::K, Trellis_::k>(i, j) &
                ((1 << Trellis_::K) - 1));
        }
    }
    return result;
...
makePrevState(Trellis_)
{
    ...
    for (size_t i = 0; i != (1 << Trellis_::K); ++i) {
        ...
        for (size_t j = 0; j != (1 << Trellis_::k); ++j) {
            size_t l =
                update_memory<Trellis_::K, Trellis_::k>(i, j) & (NumStates - 1);
            result[l][k] = i;
        }
    }
    return result;
...
constexpr auto makeCost(Trellis_ trellis)
{
    ...
    for (uint32_t i = 0; i != NumStates; ++i) {
        for (uint32_t j = 0; j != NumOutputs; ++j) {
            ...
            result[i][j] =
                to_int<int8_t, LLR>(((bit << 1) - 1) * ((1 << (LLR - 1)) - 1));
        }
    }
    return result;
}
 ...
 *
 */
template <typename Trellis_, size_t LLR_ = 2> struct Viterbi {
    ...
    Viterbi(Trellis_ trellis)
        : cost_(makeCost<Trellis_, LLR_>(trellis)),
          nextState_(makeNextState(trellis)), prevState_(makePrevState(trellis))
    {
    }

    void calculate_path_metric(const std::array<int16_t, NumStates / 2> &cost0,
                               const std::array<int16_t, NumStates / 2> &cost1,
                               std::bitset<NumStates> &hist, size_t j)
    {
    ...
    size_t decode(std::array<int8_t, IN> const &in,
                  std::array<uint8_t, OUT> &out)
    {
        static_assert(sizeof(history_) >= IN / 2);

        constexpr auto MAX_METRIC =
            std::numeric_limits<typename metrics_t::value_type>::max() / 2;
